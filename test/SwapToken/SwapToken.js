...
async function approveTokens() {
    ...
    // Kiểm tra & cấp quyền cho WA0GI (WA0GI)
...
async function swapA0GIForExactTokens() {
    console.log("\n🔄 Thực hiện swap A0GI for exact tokens...");

    const userAddress = await signer.getAddress();
    const amountOut = ethers.utils.parseUnits("100", 18);  // Số lượng token bạn muốn nhận
    const path = [WA0GI, TOKEN_B];  // Đường dẫn từ WA0GI đến token đích (ví dụ: A0GIW)
    const deadline = Math.floor(Date.now() / 1000) + 60 * 10;  // Hạn chót (10 phút từ bây giờ)

    try {
        // Lấy số lượng A0GI cần thiết để swap
        const amountsIn = await routerContract.getAmountsIn(amountOut, path);
        const amountA0GIIn = amountsIn[0];
        console.log(`📌 Cần A0GI: ${ethers.utils.formatUnits(amountA0GIIn, 18)} A0GI`);

        // Kiểm tra số dư A0GI người dùng có
        const balanceA0GI = await provider.getBalance(userAddress);
        console.log(`📌 Số dư A0GI hiện tại: ${ethers.utils.formatUnits(balanceA0GI, 18)} A0GI`);

        if (balanceA0GI.lt(amountA0GIIn)) {
            console.error("❌ Số dư A0GI không đủ!");
            return;
        }
