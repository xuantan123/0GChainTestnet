...
async function main() {
  ...
  console.log(`🔍 Kiểm tra thông tin User: ${userAddress}`);

  // 🛠 Lấy thông tin User từ contract
  const userInfo = await ZetaStake.userInfo(userAddress);

  console.log(`📌 Shares của User: ${userInfo.shares.toString()}`);
  console.log(`📌 Thời gian gửi tiền lần cuối: ${new Date(userInfo.lastDepositedTime * 1000).toLocaleString()}`);
  console.log(`📌 ZTW tại lần tương tác cuối: ${ethers.utils.formatUnits(userInfo.cakeAtLastUserAction, 18)} ZTW`);
  console.log(`📌 Thời gian tương tác cuối: ${new Date(userInfo.lastUserActionTime * 1000).toLocaleString()}`);
  console.log(`📌 Lock Start Time: ${new Date(userInfo.lockStartTime * 1000).toLocaleString()}`);
  console.log(`📌 Lock End Time: ${new Date(userInfo.lockEndTime * 1000).toLocaleString()}`);
  console.log(`📌 Boosted Shares của User: ${userInfo.userBoostedShare.toString()}`);
  console.log(`📌 Locked trạng thái: ${userInfo.locked}`);
  console.log(`📌 Locked Amount: ${ethers.utils.formatUnits(userInfo.lockedAmount, 18)} ZTW`);

  // 🔍 Kiểm tra nếu User đang bị khóa
  if (userInfo.locked) {
    const currentTime = Math.floor(Date.now() / 1000); // Thời gian hiện tại (Unix timestamp)
    console.log(`🕒 Thời gian hiện tại: ${new Date(currentTime * 1000).toLocaleString()}`);

    if (currentTime >= userInfo.lockEndTime) {
      console.log("✅ Khoảng thời gian khóa đã hết, User có thể được mở khóa!");
    } else {
      const remainingTime = userInfo.lockEndTime - currentTime;
      console.log(`⏳ User vẫn đang bị khóa. Còn ${remainingTime} giây nữa mới hết thời gian khóa.`);
    }
  } else {
    console.log("🔓 User hiện tại không bị khóa!");
  }
}
