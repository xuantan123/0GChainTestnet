...
const provider = new ethers.providers.JsonRpcProvider(process.env.URL); // RPC của A0GIChain
...
async function checkOrCreatePair() {
    console.log("\n🔍 Kiểm tra cặp thanh khoản WA0GI-A0GIW...");
...
async function approveTokens() {
    ...
    // Kiểm tra & cấp quyền cho TOKEN_A (WA0GI)
    let allowanceA = await tokenContractA.allowance(userAddress, ROUTER_ADDRESS);
    console.log(`🔎 Quyền hạn hiện tại của WA0GI: ${allowanceA.toString()}`);

    if (allowanceA.lt(amount)) {
        console.log("🔄 Đang approve WA0GI...");
        try {
            let txA = await tokenContractA.approve(ROUTER_ADDRESS, amount);
            console.log(`📌 Giao dịch approve WA0GI: ${txA.hash}`);
            await txA.wait();
            console.log("✅ WA0GI đã được cấp quyền!");
        } catch (error) {
            console.error("❌ Lỗi khi approve WA0GI:", error);
        }
    } else {
        console.log("✅ WA0GI đã được approve sẵn!");
    }

    // Kiểm tra & cấp quyền cho TOKEN_B (A0GIW)
    let allowanceB = await tokenContractB.allowance(userAddress, ROUTER_ADDRESS);
    console.log(`🔎 Quyền hạn hiện tại của A0GIW: ${allowanceB.toString()}`);

    if (allowanceB.lt(amount)) {
        console.log("🔄 Đang approve A0GIW...");
        try {
            let txB = await tokenContractB.approve(ROUTER_ADDRESS, amount);
            console.log(`📌 Giao dịch approve A0GIW: ${txB.hash}`);
            await txB.wait();
            console.log("✅ A0GIW đã được cấp quyền!");
        } catch (error) {
            console.error("❌ Lỗi khi approve A0GIW:", error);
        }
    } else {
        console.log("✅ A0GIW đã được approve sẵn!");
    }
}
...
async function checkTokenBalances() {
    ...
    console.log(`📌 Số dư WA0GI: ${ethers.utils.formatUnits(balanceA, 18)}`);
    console.log(`📌 Số dư A0GIW: ${ethers.utils.formatUnits(balanceB, 18)}`);
    
    const amountA = ethers.utils.parseUnits("2", 18);
    const amountB = ethers.utils.parseUnits("10000", 18);
    
    if (balanceA.lt(amountA)) {
        console.error(`❌ Không đủ WA0GI. Cần: ${ethers.utils.formatUnits(amountA, 18)}, Có: ${ethers.utils.formatUnits(balanceA, 18)}`);
        return false;
    }
    
    if (balanceB.lt(amountB)) {
        console.error(`❌ Không đủ A0GIW. Cần: ${ethers.utils.formatUnits(amountB, 18)}, Có: ${ethers.utils.formatUnits(balanceB, 18)}`);
        return false;
    }
...
async function addLiquidityA0GI() {
    console.log("\n💧 Thử thêm thanh khoản với A0GI gốc...");
    ...
    const amountTokenMin = ethers.utils.parseUnits("100", 18);  // 10% của 10 A0GIW
    const amountA0GIMin = ethers.utils.parseUnits("0.0001", 18); // 10% của 0.0001 A0GI
    
    // Tăng deadline lên 24 giờ
    const deadline = Math.floor(Date.now() / 1000) + 60 * 60 * 24;

    console.log("📌 Chi tiết giao dịch A0GI:");
    console.log(`- TOKEN: ${TOKEN_B}`);
    console.log(`- A0GIW gửi vào: ${ethers.utils.formatUnits(amountTokenDesired, 18)}`);
    console.log(`- A0GI gửi vào: ${ethers.utils.formatUnits(amountA0GI, 18)}`);
...
// npx hardhat run test/AddLiquidity/AddLiquidity.js --network ZeroGravityChain